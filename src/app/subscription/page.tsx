...
export default function SubscriptionPage() {
  ...
  const [selectedDeliveryDays, setSelectedDeliveryDays] = useState<DeliveryDay[]>([]);
  ...
  useEffect(() => {
    ...
    if (selectedPlan && selectedMealTypes.length > 0 && selectedDeliveryDays.length > 0) {
      const calculatedPrice = selectedPlan.price * selectedMealTypes.length * selectedDeliveryDays.length * 4.3;
  ...
  }, [customerName, phoneNumber, selectedPlanId, selectedMealTypes, selectedDeliveryDays]);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    alert('Formulir langganan berhasil disubmit! (Data masih tercetak di konsol. Integrasi backend akan dilakukan di langkah selanjutnya.)');
  ...
  return (
    <div className="container mx-auto p-8 py-12">
      ...
      <form onSubmit={handleSubmit} className="bg-white p-10 rounded-xl shadow-lg border border-gray-200 max-w-3xl mx-auto space-y-8">
        {/* Nama Lengkap */}
        <div className="grid w-full items-center gap-1.5">
          <Label htmlFor="customerName">Nama Lengkap <span className="text-red-500">*</span></Label>
          <Input
            type="text"
            id="customerName"
        ...
        <div className="grid w-full items-center gap-1.5">
          <Label htmlFor="phoneNumber">Nomor Telepon Aktif <span className="text-red-500">*</span></Label>
          <Input
            type="tel"
            id="phoneNumber"
        ...
        <div className="grid w-full items-center gap-1.5">
          <Label htmlFor="planSelection">Pilih Paket <span className="text-red-500">*</span></Label>
        ...
        <div className="grid w-full gap-2">
          <Label>Jenis Makanan <span className="text-red-500">*</span></Label>
            ...
            {allMealTypes.map((type) => (
              <div key={type} className="flex items-center space-x-2">
                <Checkbox
                  ...
                  onCheckedChange={(checked) => handleMealTypeChange(type, Boolean(checked))}
                />
                <Label htmlFor={`meal-type-${type}`} className="cursor-pointer">{type}</Label>
              </div>
            ))}
          </div>
          {selectedMealTypes.length === 0 && (
            <p className="text-red-500 text-sm mt-1">Setidaknya satu jenis makanan harus dipilih.</p>
          )}
        </div>

        <div className="grid w-full gap-2">
          <Label>Hari Pengiriman <span className="text-red-500">*</span></Label>
            ...
            {allDeliveryDays.map((day) => (
              <div key={day} className="flex items-center space-x-2">
                <Checkbox
                  ...
                  onCheckedChange={(checked) => handleDeliveryDayChange(day, Boolean(checked))}
                />
                <Label htmlFor={`delivery-day-${day}`} className="cursor-pointer">{day}</Label>
              </div>
            ))}
          </div>
          {selectedDeliveryDays.length === 0 && (
            <p className="text-red-500 text-sm mt-1">Setidaknya satu hari pengiriman harus dipilih.</p>
          )}
        </div>

        <div className="grid w-full gap-1.5">
          <Label htmlFor="allergies">Alergi / Pembatasan Diet Lainnya (Opsional)</Label>
          <Textarea
            id="allergies"
        ...
        <div className="bg-emerald-50 p-6 rounded-lg text-center border border-emerald-200">
          <h3 className="text-2xl font-bold text-emerald-800 mb-2">Total Harga Langganan:</h3>
